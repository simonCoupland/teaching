...
float Vector2D::crossProduct(Vector2D& other) //!< Returns the cross product of this vector with other
{
	return (data[0] * other.data[1] - data[1] * other.data[0]);
}

Vector2D Vector2D::crossProduct(float scalar /* Some scalar quantity */)
{
	Vector2D result(scalar * data[1], -scalar * data[0]);
	return result;
}
